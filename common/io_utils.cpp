...
void LSH_parse(int argc, char * const argv[], LSH_Args **args) {
    ...
    while ( (opt = getopt(argc, argv, "m:d:q:k:L:o:N:R:l:")) != -1 ) {
        switch (opt) {
            case 'm':
                if(atoi(optarg) == 2) {
                        const char* program = "./mrngR";
                        int opt2{};
                        const char* args[7];
                        optind=1;
                        while ( (opt2 = getopt(argc, argv, "m:d:q:k:L:o:N:R:l:")) != -1 ){
                            printf("opt2: %c\n", opt2);
                            switch (opt2) {
                                case 'k':
                                case 'L': 
                        ...
                        /*const char* args[] = {
                            program,
                            "-d", argv[optind + 1],  // Assuming 'd' option is used for input_file
                            "-q", argv[optind + 3],  // Assuming 'q' option is used for query_file
                            "-N", argv[optind + 5],  // Assuming 'N' option is used for 100
                            "-l", argv[optind + 4],  // Assuming 'l' option is used for 5
                            "-o", argv[optind + 6]  // Assuming 'o' option is used for output_
                        };*/
                        //print args
                        args[0] = program;
                        for (int i = 0; i < 6; i++) {    
